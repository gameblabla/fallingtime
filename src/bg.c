...
#ifdef EMBED
	rw = SDL_RWFromMem(IMG_ICEBERGS, ICEBERGS_SIZE);
	tmp = SDL_LoadBMP_RW(rw, 0);
	SDL_FreeRW(rw);
#else
	tmp = SDL_LoadBMP(IMAGE_ICEBERGS);
#endif
	SDL_SetColorKey(tmp, (SDL_SRCCOLORKEY | SDL_ACCELERATION_RLE), 0);
	bg->Icicles.S = SDL_DisplayFormat(tmp);
	SDL_FreeSurface(tmp);
	
#ifdef EMBED
	rw = SDL_RWFromMem(IMG_FLARE, FLARE_SIZE);
	tmp = SDL_LoadBMP_RW(rw, 0);
	SDL_FreeRW(rw);
#else
	tmp = SDL_LoadBMP(IMAGE_FLARE);
#endif
	SDL_SetColorKey(tmp, (SDL_SRCCOLORKEY | SDL_ACCELERATION_RLE), 0);
	bg->Flares.S = SDL_DisplayFormat(tmp);
	SDL_FreeSurface(tmp);

#ifdef EMBED
	rw = SDL_RWFromMem(IMG_STARS, STARS_SIZE);
	tmp = SDL_LoadBMP_RW(rw, 0);
	SDL_FreeRW(rw);
#else	
	tmp = SDL_LoadBMP(IMAGE_STARS);
#endif
	SDL_SetColorKey(tmp, (SDL_SRCCOLORKEY | SDL_ACCELERATION_RLE), 0);
	bg->Stars.S = SDL_DisplayFormat(tmp);
	SDL_FreeSurface(tmp);
